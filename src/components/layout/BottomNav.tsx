...
import { Clock, ClipboardList, CircleDot, User, Settings2 } from 'lucide-react';
...
export function BottomNav() {
    ...
    if (isAdmin) {
        items.push({ name: 'Sistema', path: '/sistema', icon: Settings2 });
    }
        ...
        <nav className="fixed bottom-0 left-0 right-0 bg-main border-t border-border px-6 py-4 flex justify-between items-center z-50">
            {items.map((item) => (
                <NavLink
                    ...
                    to={item.path}
                    className={({ isActive }) =>
                        `flex flex-col items-center gap-1 transition-colors duration-200 min-w-[56px] ${isActive ? 'text-accent' : 'text-secondary hover:text-primary'
                        }`
                    }
                >
